...
def tir(choix_case, grille_adversaire_bateaux, grille_joueur_bateaux_ennemis, bateaux_vivants_ennemi, client):
...
def valider_coordonne(event=""):
    global infos_bateaux, coordonnees_bateaux, iden_bateaux, bateaux_vivants_joueur, grille_joueur_bateaux, bateaux_places, fenetre, etiquette_joueur, bouton_valider
    if bateaux_places < 5:
...
def tour_joueur(event):
    ...
    global game, grille_joueur_bateaux, grille_joueur_bateaux_ennemis, grille_adversaire_bateaux, bateaux_vivants_ennemi, client
    ...
    if game:
        ...
        if grille_joueur_bateaux_ennemis[lig][col] != ' ~':
            ...
            resultat, grille_adversaire_bateaux, grille_joueur_bateaux_ennemis, bateaux_vivants_ennemi = tir(
                choix_case, grille_adversaire_bateaux, grille_joueur_bateaux_ennemis, bateaux_vivants_ennemi, client)
...
def tour_adversaire():
    ...
    global game, grille_joueur_bateaux, grille_joueur_bateaux_ennemis, grille_adversaire_bateaux, bateaux_vivants_joueur, plateau_bateaux, case_vers_identifiant_bateaux_ennemis, identifiant_vers_case_bateaux_ennemis, client
    ...
    if game:
        ...
        while resultat == True:
            ...
            while resultat == "erreur":
                choix_case = client.recv(25000)
                choix_case=choix_case.decode()
...
def plateaux_jeux(fenetre):
    ...
    client.recv(500).decode()
    bateaux_vivants_ennemi, grille_adversaire_bateaux = envoi_recu_donnees(client, bateaux_vivants_joueur, grille_joueur_bateaux)
    ...
    client.send(str(premier_joueur).encode())

    if premier_joueur:
        showinfo("Info", "Vous commencez à jouer")
    else:
        showinfo("Info", "L'adversaire joue en premier")
        tour_adversaire()

def envoi_recu_donnees(client, bateaux_vivants_joueur, grille_joueur_bateaux):
    #On envoie les variables à l'adversaire correspondant à nos caractéristiques
    ...
    #On recupere les variables correspondant aux caracteristiques de l'adversaire
    donnees = client.recv(250000)
...
#On ouvre le serveur et on attend que le client s'y connecte
...
fenetre_info = Tk()
fenetre_info.title("Connexion")
fenetre_info.geometry("350x150+500+250")
fenetre_info['bg'] = "#F0EBCA"

label_regles1 = Label(fenetre_info, text=f"Votre adversaire doit entrer l'adresse\nIP : {IP} pour pouvoir se connecter", bg="#F0EBCA", fg="#786c58", justify="center", wraplength="500", font=("Arial",13))
label_regles1.grid(row = 0, column = 0, padx = 15, pady =10)

bouton_fermer = Button(fenetre_info, text='Fermer', anchor='center', command=fenetre_info.destroy, bg="#e7d5b8", fg="#786c58")
bouton_fermer.grid(row=1, column=0, padx=15, pady=10)
...
fenetre_info.after(100, connexion)
...
#On defini l'emplacement des bateaux
